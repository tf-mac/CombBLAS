...
int main(int argc, char* argv[])
{
    ...
    if (argc < 4) {
        if (myrank == 0) cout << "Usage: ./BlockedSpGEMM <MatrixA> <MatrixB> <br> <bc>" << endl;
        MPI_Finalize();
        return -1;
    }

    {
        ...
        while (bspgemm.hasNext()) {
            ...
            if (myrank == 0)
                cout << "block size " << nr << " " << nc << " " << nnz << " offsets " << roffset << " " << coffset
                     << std::endl;
        }
