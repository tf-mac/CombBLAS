...
std::vector<RT>
findColSplitters(SpTuples<IT, NT>*& spTuples, int nsplits)
{
...
template <typename RT, typename IT, typename NT>
std::vector<RT>
findColSplittersFinger(SpTuples<IT, NT>*& spTuples, int nsplits)
{
    std::vector<RT> splitters(nsplits + 1);
    splitters[0] = static_cast<RT>(0);
    ColLexiCompare<IT, NT> comp;
...
IT
SerialMergeNNZ(const std::vector<SpTuples<IT, NT>*>& ArrSpTups)
{
...
void
SerialMerge(const std::vector<SpTuples<IT, NT>*>& ArrSpTups, std::tuple<IT, IT, NT>* ntuples)
{
    int nlists = ArrSpTups.size();
    ColLexiCompare<IT, int> heapcomp;
...
template <class IT, class NT>
IT*
SerialMergeNNZHash(const std::vector<SpTuples<IT, NT>*>& ArrSpTups, IT& totnnz, IT& maxnnzPerCol, IT startCol, IT endCol)
{
    int nlists = ArrSpTups.size();
...
template <class SR, class IT, class NT>
void
SerialMergeHash(const std::vector<SpTuples<IT, NT>*>& ArrSpTups, std::tuple<IT, IT, NT>* ntuples, IT* colnnz, IT maxcolnnz, IT startCol, IT endCol,
                bool sorted)
{
    int nlists = ArrSpTups.size();
    IT ncols = endCol - startCol;  // in this split
...
template <class SR, class IT, class NT>
SpTuples<IT, NT>*
MultiwayMerge(std::vector<SpTuples<IT, NT>*>& ArrSpTups, IT mdim = 0, IT ndim = 0, bool delarrs = false)
{
    int nlists = ArrSpTups.size();
    if (nlists == 0) {
...
template <class SR, class IT, class NT>
SpTuples<IT, NT>*
MultiwayMergeHash(std::vector<SpTuples<IT, NT>*>& ArrSpTups, IT mdim = 0, IT ndim = 0, bool delarrs = false, bool sorted = true)
{
    int nprocs, myrank;
    ...
    if (nlists == 1) {
        ...
        {
            std::tuple<IT, IT, NT>* mergeTups =
                static_cast<std::tuple<IT, IT, NT>*>(::operator new(sizeof(std::tuple<IT, IT, NT>[ArrSpTups[0]->getnnz()])));
#ifdef THREADED
#pragma omp parallel for
#endif
            for (int i = 0; i < ArrSpTups[0]->getnnz(); i++) mergeTups[i] = ArrSpTups[0]->tuples[i];
...
template <class SR, class IT, class NT>
SpTuples<IT, NT>*
MultiwayMergeHashSliding(std::vector<SpTuples<IT, NT>*>& ArrSpTups, IT mdim = 0, IT ndim = 0, bool delarrs = false, bool sorted = true,
