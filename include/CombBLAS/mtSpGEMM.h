...
template <typename T>
T*
prefixsum(T* in, int size, int nthreads)
{
...
SpTuples<IT, NTO>*
LocalSpGEMM(const SpDCCols<IT, NT1>& A, const SpDCCols<IT, NT2>& B, bool clearA, bool clearB)
{
    IT mdim = A.getnrow();
    IT ndim = B.getncol();
    IT nnzA = A.getnnz();
...
bool
sort_less(const std::pair<IT, NT>& left, const std::pair<IT, NT>& right)
{
...
template <typename SR, typename NTO, typename IT, typename NT1, typename NT2>
SpTuples<IT, NTO>*
LocalHybridSpGEMM(const SpDCCols<IT, NT1>& A, const SpDCCols<IT, NT2>& B, bool clearA, bool clearB, IT* aux = nullptr)
{
    IT mdim = A.getnrow();
    IT ndim = B.getncol();
    IT nnzA = A.getnnz();
...
#pragma omp parallel for
    ...
    for (size_t i = 0; i < Bdcsc->nzc; ++i) {
        ...
        {
            if (globalHeapVecAll[myThread].size() < nnzcolB) globalHeapVecAll[myThread].resize(nnzcolB);
        ...
        else  // Hash Algorithm
        {
            ...
            if (globalHashVecAll[myThread].size() < ht_size) globalHashVecAll[myThread].resize(ht_size);
            // std::vector<HeapEntry<IT,NT1>> globalheapVec(nnzcolB);
            // HeapEntry<IT, NT1> * wset = globalheapVec.data();
            // HeapEntry<IT, NT1> * wset = globalheapVecAll[myThread].data();
...
template <typename SR, typename NTO, typename IT, typename NT1, typename NT2>
SpTuples<IT, NTO>*
LocalHybridSpGEMM_CUDA(const SpDCCols<IT, NT1>& A, const SpDCCols<IT, NT2>& B, bool clearA, bool clearB, IT* aux = nullptr)
{
    IT mdim = A.getnrow();
    IT ndim = B.getncol();
    IT nnzA = A.getnnz();
...
template <typename SR, typename NTO, typename IT, typename NT1, typename NT2>
SpTuples<IT, NTO>*
LocalSpGEMMHash(const SpDCCols<IT, NT1>& A, const SpDCCols<IT, NT2>& B, bool clearA, bool clearB, bool sort = true)
{
    double t0 = MPI_Wtime();
...
IT
EstimateLocalFLOP(const SpDCCols<IT, NT1>& A, const SpDCCols<IT, NT2>& B, bool clearA, bool clearB)
{
    Dcsc<IT, NT1>* Adcsc = A.GetDCSC();
    Dcsc<IT, NT2>* Bdcsc = B.GetDCSC();
    int numThreads = 1;
...
IT*
estimateNNZ(const SpDCCols<IT, NT1>& A, const SpDCCols<IT, NT2>& B, IT* aux = nullptr, bool freeaux = true)
{
    IT nnzA = A.getnnz();
    if (A.isZero() || B.isZero()) {
        return NULL;
...
template <typename IT, typename NT1, typename NT2>
IT*
estimateNNZ_Hash(const SpDCCols<IT, NT1>& A, const SpDCCols<IT, NT2>& B, IT* flopC, IT* aux = nullptr)
{
    IT nnzA = A.getnnz();
    if (A.isZero() || B.isZero()) {
        return NULL;
...
template <typename IT, typename NT1, typename NT2>
int64_t
estimateNNZ_sampling(const SpDCCols<IT, NT1>& A, const SpDCCols<IT, NT2>& B, int nrounds = 5)
{
    IT nnzA = A.getnnz();
...
#pragma omp parallel for
#endif
        for (IT i = 0; i < m * nrounds; ++i) samples_init[i] = exp_dist(gen);
    }

#ifdef THREADED
#pragma omp parallel for
#endif
    for (IT i = 0; i < p * nrounds; ++i) samples_mid[i] = std::numeric_limits<float>::max();
    ...
    for (IT i = 0; i < Adcsc->nzc; ++i) {
        ...
        for (IT j = Adcsc->cp[i]; j < Adcsc->cp[i + 1]; ++j) {
            ...
            for (int k = 0; k < nrounds; ++k) {
                if (samples_init[beg_init + k] < samples_mid[beg_mid + k]) samples_mid[beg_mid + k] = samples_init[beg_init + k];
            }
        }
    }
...
#pragma omp parallel for reduction(+ : nnzest)
    ...
    for (IT i = 0; i < Bdcsc->nzc; ++i) {
        ...
        for (IT k = beg_final; k < beg_final + nrounds; ++k) samples_final[k] = std::numeric_limits<float>::max();

        for (IT j = Bdcsc->cp[i]; j < Bdcsc->cp[i + 1]; ++j) {
            IT row = Bdcsc->ir[j];
            IT beg_mid = row * nrounds;
            for (int k = 0; k < nrounds; ++k) {
                if (samples_mid[beg_mid + k] < samples_final[beg_final + k]) samples_final[beg_final + k] = samples_mid[beg_mid + k];
            }
        }

        colest[i] = 0.0f;
        for (IT k = beg_final; k < beg_final + nrounds; ++k) colest[i] += samples_final[k];
...
template <typename IT, typename NT1, typename NT2>
IT*
estimateFLOP(const SpDCCols<IT, NT1>& A, const SpDCCols<IT, NT2>& B, IT* aux = nullptr)
{
    IT nnzA = A.getnnz();
    if (A.isZero() || B.isZero()) {
        return NULL;
...
template <typename SR, typename NTO, typename IT, typename NT1, typename NT2>
SpTuples<IT, NTO>*
LocalHybridSpGEMM(const SpCCols<IT, NT1>& A, const SpCCols<IT, NT2>& B, bool clearA, bool clearB)
{
    double t0 = MPI_Wtime();
    ...
    if (A.isZero() || B.isZero()) return new SpTuples<IT, NTO>(0, mdim, ndim);
...
#pragma omp parallel for
    ...
    for (size_t i = 0; i < Bcsc->n; ++i) {
        ...
        if (cr < 2.0)  // Heap Algorithm
        {
            ...
            for (IT j = Bcsc->jc[i]; j < Bcsc->jc[i + 1]; ++j) {
                ...
                if (Acsc->jc[ca] != Acsc->jc[ca + 1])  // col not empty
                    wset[hsize++] = HeapEntry<IT, NT1>(Acsc->ir[Acsc->jc[ca]], j, Acsc->num[Acsc->jc[ca]]);
            }
            ...
            while (hsize > 0) {
                ...
                if (!SR::returnedSAID()) {
                    if ((curptr > colptrC[i]) && std::get<0>(tuplesC[curptr - 1]) == wset[hsize - 1].key)
                        ...
                        tuplesC[curptr++] = std::make_tuple(wset[hsize - 1].key, i, mrhs);
                }
        ...
        else  // Hash Algorithm
        {
            ...
            while (ht_size < nnzcolC) ht_size <<= 1;
            std::vector<std::pair<IT, NTO>> T(ht_size);

            for (size_t j = 0; j < ht_size; ++j) T[j].first = std::numeric_limits<IT>::max();
            ...
            size_t index = 0;
            for (size_t j = 0; j < ht_size; ++j) {
                if (T[j].first != std::numeric_limits<IT>::max()) T[index++] = T[j];
            }

            std::sort(T.begin(), T.begin() + index, sort_less<IT, NTO>);

            IT curptr = colptrC[i];
            for (size_t j = 0; j < index; ++j) tuplesC[curptr++] = std::make_tuple(T[j].first, i, T[j].second);
        }
    }

    if (clearA) delete const_cast<SpCCols<IT, NT1>*>(&A);
    if (clearB) delete const_cast<SpCCols<IT, NT2>*>(&B);
...
template <typename IT, typename NT1, typename NT2>
IT*
estimateFLOP(const SpCCols<IT, NT1>& A, const SpCCols<IT, NT2>& B)
{
    IT nnzA = A.getnnz();
...
#pragma omp parallel for
#endif
    for (IT i = 0; i < Bcsc->n; ++i) colflopC[i] = 0;

#ifdef THREADED
#pragma omp parallel for
#endif
    for (IT i = 0; i < Bcsc->n; ++i) {
        for (IT j = Bcsc->jc[i]; j < Bcsc->jc[i + 1]; ++j) colflopC[i] += Acsc->jc[Bcsc->ir[j] + 1] - Acsc->jc[Bcsc->ir[j]];
    }
...
template <typename IT, typename NT1, typename NT2>
IT*
estimateNNZ_Hash(const SpCCols<IT, NT1>& A, const SpCCols<IT, NT2>& B, const IT* flopC)
{
    IT nnzA = A.getnnz();
    if (A.isZero() || B.isZero()) return NULL;
...
#pragma omp parallel for
#endif
    for (IT i = 0; i < Bcsc->n; ++i) colnnzC[i] = 0;
...
#pragma omp parallel for
    ...
    for (IT i = 0; i < Bcsc->n; ++i) {
        ...
        for (IT j = 0; (unsigned)j < ht_size; ++j) T[j] = std::numeric_limits<IT>::max();
