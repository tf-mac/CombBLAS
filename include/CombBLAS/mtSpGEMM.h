...
#pragma omp parallel
    ...
    for (int i = 0; i < numThreads;
         i++)  // inital allocation per thread, may be an overestimate, but does not require more memoty than inputs
    {
...
#pragma omp parallel
    ...
    /*
    for(int i=0; i<numThreads; i++) //inital allocation per thread, may be an overestimate, but does not require more
    memoty than inputs
    {
        colindsVec[i].resize(nnzA/numThreads);
    }*/
...
#pragma omp parallel for
    ...
    for (size_t i = 0; i < Bdcsc->nzc; ++i) {
        ...
        else  // Hash Algorithm
        {
            ...
            // colinds.first vector keeps indices to A.cp, i.e. it dereferences "colnums" vector (above),
            // colinds.second vector keeps the end indices (i.e. it gives the index to the last valid element of
            // A.cpnack)
            ...
            std::sort(globalHashVecAll[myThread].begin(), globalHashVecAll[myThread].begin() + index,
                      sort_less<IT, NTO>);
            IT curptr = colptrC[i];
...
#pragma omp parallel
    ...
    // std::tuple<IT,IT,NTO> * tuplesC = static_cast<std::tuple<IT,IT,NTO> *> (::operator new
...
#pragma omp parallel for
    ...
    for (int i = 0; i < Bdcsc->nzc; ++i) {
        ...
        while (hsize > 0) {
            std::pop_heap(curheap, curheap + hsize,
                          std::greater<std::pair<IT, IT>>());  // result is stored in wset[hsize-1]
