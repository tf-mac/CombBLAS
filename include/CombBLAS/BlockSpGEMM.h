...
struct BlockSpGEMM {
   ...
   public:
    BlockSpGEMM(SpParMat<IT, NTA, DERA> &A, SpParMat<IT, NTB, DERB> &B, int br, int bc, int bi = 1) : br_(br), bc_(bc), bi_(bi), cur_block_(0)
    {
    ...
    SpParMat<IT, NTC, DERC> getNextBlock(IT &roffset, IT &coffset)
    {
        ...
        IT bs = nr_ / br_;
        IT r = nr_ % br_;
        roffset = (std::min(static_cast<IT>(rbid), r) * (bs + 1)) + ((rbid < r ? 0 : rbid - r) * bs);
        // (std::max(static_cast<IT>(0), rbid-r)*bs);

        bs = nc_ / bc_;
        r = nc_ % bc_;
        coffset = (std::min(static_cast<IT>(cbid), r) * (bs + 1)) + ((cbid < r ? 0 : cbid - r) * bs);
        // (std::max(static_cast<IT>(0), cbid-r)*bs);

        return Mult_AnXBn_DoubleBuff<SR, NTC, DERC>(A_blocks_[rbid][0], B_blocks_[0][cbid], false, false);
    }

    bool hasNext() { return cur_block_ < br_ * bc_; }
    ...
    std::vector<IT> getBlockOffsets(bool is_row)
    {
        ...
        for (int b = 0; b < nblocks; ++b) offsets[b] = (std::min(static_cast<IT>(b), r) * (bs + 1)) + ((b < r ? 0 : b - r) * bs);
