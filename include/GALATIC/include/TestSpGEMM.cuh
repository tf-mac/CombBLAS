...
void TestSpGEMM(dCSR<typename SEMIRING_T::leftInput_t>& A, dCSR<typename SEMIRING_T::rightInput_t>& B,
                SEMIRING_T semiring, F equiv_rel, GPUMatrixMatrixMultiplyTraits& traits)
{
    ...
    for (int i = 0; i < CPU_result_cpu.rows + 1; i++) {
        if (CPU_result_cpu.row_offsets[i] == GPU_result_cpu.row_offsets[i]) {
            ...
            std::cout << " issue at " << i << " with " << CPU_result_cpu.row_offsets[i] << " vs  "
                      << GPU_result_cpu.row_offsets[i] << std::endl;
        }
    }
