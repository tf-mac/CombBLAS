...
//
#include "CombBLAS/PerfRecorder.hpp"
...
void PerformanceRecorder::RegisterDoubleMatrix(std::string printinfo, std::vector<int> shape, int printspace, double scale)
{
...
void PerformanceRecorder::RegisterIntegerMatrix(std::string printinfo, std::vector<int> shape, int printspace, double scale)
{
...
std::string PerformanceRecorder::GatherLocalString(std::string localstr) const
{
    ...
    MPI_Allgatherv(localstr.data(), localstr.size(), MPI_CHAR, tmpc, recvsize.data(), recvdisp.data(), MPI_CHAR, MPI_COMM_WORLD);
    return ret;
}
...
void PerformanceRecorder::PrintScalarInternal(std::ostream& os)
{
    ...
    for (auto key : printseq) {
        if (doublemap.find(key) != doublemap.end()) {
            ...
            tmpos << std::defaultfloat << std::setprecision(5) << std::setw(doublekeys[j].size() + printspacemap[doublekeys[j]] + 1) << std::left
        ...
        if (integermap.find(key) != integermap.end()) {
            ...
            tmpos << std::setprecision(3) << std::scientific << std::setw(integerkeys[j].size() + printspacemap[integerkeys[j]] + 1) << std::left
