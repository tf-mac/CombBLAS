...
#pragma omp parallel for
    ...
    for (int i = 1; i < nsplits; i++) {
        ...
        std::tuple<IT, IT, NT>* it =
            std::lower_bound(spTuples->tuples, spTuples->tuples + spTuples->getnnz(), search_tuple, comp);
...
IT SerialMergeNNZ(const std::vector<SpTuples<IT, NT>*>& ArrSpTups)
{
    ...
    for (int i = 0; i < nlists; ++i) {
        if (ArrSpTups[i]->getnnz() > 0) {
            heap[hsize++] =
                std::make_tuple(std::get<0>(ArrSpTups[i]->tuples[0]), std::get<1>(ArrSpTups[i]->tuples[0]), i);
        }
    }
    ...
    while (hsize > 0) {
        ...
        if ((estnnz == 0) || (std::get<0>(curTuple) != std::get<0>(heap[hsize - 1])) ||
            (std::get<1>(curTuple) != std::get<1>(heap[hsize - 1]))) {
                       ...
                       IT endCol)
{
    ...
    for (IT col = 0; col < ncols; col++) {
        ...
        if (globalHashVec.size() < ht_size) globalHashVec.resize(ht_size);
...
#pragma omp parallel for
#endif
            for (int i = 0; i < ArrSpTups[0]->getnnz(); i++) mergeTups[i] = ArrSpTups[0]->tuples[i];
...
#pragma omp parallel for schedule(dynamic)
    ...
    for (int i = 0; i < nsplits; ++i) mdisp[i + 1] = mdisp[i] + mergedNnzPerSplit[i];
    ...
    // std::tuple<IT, IT, NT> * mergeBuf = static_cast<std::tuple<IT, IT, NT>*> (::operator new (sizeof(std::tuple<IT,
    // IT, NT>[mergedNnzAll])));
...
#pragma omp parallel for schedule(dynamic)
    ...
    for (int i = 0; i < nlists; i++) {
        if (delarrs) delete ArrSpTups[i];  // May be expensive for large local matrices
    }
...
#pragma omp parallel for schedule(dynamic)
    ...
    // ------ allocate memory outside of the parallel region ------
    std::tuple<IT, IT, NT>* mergeBuf =
        static_cast<std::tuple<IT, IT, NT>*>(::operator new(sizeof(std::tuple<IT, IT, NT>[mergedNnzAll])));
    // std::tuple<IT, IT, NT> * mergeBuf = new std::tuple<IT, IT, NT>[mergedNnzAll];
...
#pragma omp parallel for schedule(dynamic)
    ...
    for (int i = 0; i < nsplits; i++)  // serially merge part by part
    {
        ...
        IT endCol = (i + 1) * (ndim / nsplits);
        if (i == (nsplits - 1)) endCol = ndim;
        SerialMergeHash<SR>(listSplitTups[i], mergeBuf + mdisp[i], nnzPerColSplit[i], maxNnzPerColumnSplit[i], startCol,
                            endCol, sorted);
...
#pragma omp parallel for
    ...
    for (int i = 0; i < nlists; ++i) {
        if ((mdim != ArrSpTups[i]->getnrow()) || ndim != ArrSpTups[i]->getncol()) {
            ...
            // std::cerr << mdim << " vs " << ArrSpTups[i]->getnrow() << " | " << ndim << " vs " <<
            // ArrSpTups[i]->getncol() << std::endl;
            return new SpTuples<IT, NT>(0, 0, 0);
        }
    }
...
#pragma omp parallel for
    ...
    for (int s = 0; s < nsplits; s++) {
        ...
        for (int l = 0; l < nlists; l++) {
            ...
            std::tuple<IT, IT, NT>* first = std::lower_bound(
                ArrSpTups[l]->tuples, ArrSpTups[l]->tuples + ArrSpTups[l]->getnnz(), firstTuple, colCmp);
            std::tuple<IT, IT, NT> lastTuple(0, endColSplit, NT());
            std::tuple<IT, IT, NT>* last = std::lower_bound(
                ArrSpTups[l]->tuples, ArrSpTups[l]->tuples + ArrSpTups[l]->getnnz(), lastTuple, colCmp);
...
#pragma omp parallel for
    ...
    std::pair<IT, IT>* windowsSymbolic =
        static_cast<std::pair<IT, IT>*>(::operator new(sizeof(std::pair<IT, IT>[prefixSumWindowSymbolic[ndim]])));
     ...
     * */
    std::pair<IT, IT>** rowIdsRange =
        static_cast<std::pair<IT, IT>**>(::operator new(sizeof(std::pair<IT, IT>* [nsplits])));
    for (int s = 0; s < nsplits; s++) {
...
#pragma omp for schedule(dynamic)
    ...
    std::pair<IT, IT>* windows =
        static_cast<std::pair<IT, IT>*>(::operator new(sizeof(std::pair<IT, IT>[prefixSumWindow[ndim]])));
...
#pragma omp parallel for schedule(dynamic)
    ...
    std::tuple<IT, IT, NT>* mergeBuf =
        static_cast<std::tuple<IT, IT, NT>*>(::operator new(sizeof(std::tuple<IT, IT, NT>[totalNnz])));
            ...
            for (IT c = startCol; c < endCol; c++) {
                ...
                if (nWindow == 1) {
                    ...
                    if (sorted) {
                        ...
                        for (size_t j = 0; j < index; j++) {
                            mergeBuf[outptr] =
                                std::tuple<IT, IT, NT>(globalHashVec[j].first, c, globalHashVec[j].second);
                            outptr++;
                        }
                    } else {
                        for (size_t j = 0; j < htSize; j++) {
